...
export default async function HolidayShowPage({
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 p-4 sm:p-6">
          ...
          <div className="flex-1 space-y-2">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <div className="inline-flex items-center gap-1.5 rounded-md bg-zinc-50 dark:bg-zinc-800/50 px-2.5 py-1 text-xs font-medium text-zinc-600 dark:text-zinc-400">
                ...
                {holiday.views_count_bn || holiday.views_count || 0} বার দেখা হয়েছে
              </div>
            </div>
          </div>
      ...
      {/* Date Card */}
      <div className="flex gap-4 p-4 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-400/10 shadow-sm">
        <div className="flex flex-col items-center justify-center w-16 h-16 rounded-xl bg-amber-50 dark:bg-amber-900/20 shrink-0">
          ...
          <span className="text-2xl font-bold leading-none text-zinc-800 dark:text-zinc-100">
            {holiday.day_numeric}
          </span>
        </div>
      ...
      {/* Large Image */}
      {holiday.image_url && (
        <div className="relative w-full h-[280px] sm:h-[400px] rounded-2xl overflow-hidden bg-zinc-100 dark:bg-zinc-800">
          <Image
      ...
      {/* Details */}
      <section aria-labelledby="holiday-details-heading" className="space-y-3">
        <h2
      ...
      <section className="rounded-2xl bg-zinc-50 dark:bg-zinc-800/40 p-5 space-y-3">
        ...
        <div className="text-sm leading-relaxed text-zinc-600 dark:text-zinc-300 space-y-3">
          <p>
            <strong>{holiday.title}</strong> হলো বাংলাদেশের{" "}
            {holiday.type || ""} ছুটি। তারিখ:{" "}
            <strong>{holiday.date_formatted}</strong> ({holiday.day_name_bn})।
          </p>
        </div>
      </section>
      ...
      <section className="space-y-3 pt-2">
        ...
        <div className="space-y-2">
          ...
          <details className="group rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-400/10 overflow-hidden">
            ...
            <div className="px-4 pb-4 text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed border-t border-zinc-400/25 pt-3">
              <Link href="/bangla/holiday" className="text-amber-600 hover:underline font-medium">
                ছুটির ক্যালেন্ডার
              </Link>{" "}
