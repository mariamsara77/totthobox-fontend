...
export default function TourismShowClient({ tourism }: Props) {
  ...
  const media =
    tourism.images && tourism.images.length > 0
      ? tourism.images.map((img: any) => ({
          url: img.url,
          caption: img.caption || tourism.title,
        }))
      : tourism.image_url
        ? [
            {
              url: tourism.image_url,
              caption: tourism.title,
            },
          ]
        : [];
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 p-4 sm:p-6">
      ...
      <nav className="flex items-center gap-2 text-sm">
        ...
        <span>/</span>
        <span className="truncate opacity-70">{tourism.title}</span>
      </nav>
      ...
      {/* ✅ Media Gallery - Multiple + Single Image (Holiday style) */}
      {media.length > 0 && (
      ...
      <section className="rounded-2xl bg-zinc-400/10 p-5 space-y-3">
        ...
        <div className="text-sm leading-relaxed space-y-2 opacity-90">
          <p>
            <strong>{tourism.title}</strong> বাংলাদেশের একটি দর্শনীয় স্থান।
            {tourism.type_label && (
              <>
                {" "}
      ...
      <section className="space-y-3">
        ...
        <details className="group rounded-xl bg-zinc-400/10 overflow-hidden">
          <summary className="flex items-center justify-between cursor-pointer px-4 py-3 list-none hover:bg-zinc-400/15 transition">
            <span className="text-sm font-medium">{tourism.title} কী?</span>
        ...
        <details className="group rounded-xl bg-zinc-400/10 overflow-hidden">
          <summary className="flex items-center justify-between cursor-pointer px-4 py-3 list-none hover:bg-zinc-400/15 transition">
            <span className="text-sm font-medium">
              অন্যান্য পর্যটন কেন্দ্র কোথায় দেখব?
            </span>
            <ChevronDown className="w-4 h-4 group-open:rotate-180 transition shrink-0" />
          </summary>
          <div className="px-4 pb-4 text-sm leading-relaxed border-t border-zinc-400/20 pt-3 opacity-90">
            <Link href="/bangladesh/tourism" className="underline">
              পর্যটন কেন্দ্র
            </Link>{" "}
