...
export default function InteractiveActions({
    ...
    async function fetchUserReaction() {
      ...
      try {
        const res = await fetch(
          `${API_BASE}/api/establishment-bd/${establishmentId}/reaction-status`,
          {
            headers: {
              ...getAuthHeaders(),
  ...
  const handleReact = async (type: "like" | "dislike") => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/api/establishment-bd/${establishmentId}/react`,
        {
          method: "POST",
          headers: {
            ...getAuthHeaders(),
  ...
  const handleShare = async () => {
    const url = `${window.location.origin}/bangladesh/establishment/${initialData.slug}`;
    try {
      if (navigator.share) {
