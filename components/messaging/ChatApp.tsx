...
function SearchUserRow({ user, active, onMessage }: { user: ChatUser; active: boolean; onMessage: () => void }) {
  ...
  return <div className={`group flex items-center gap-2 px-3 py-2.5 transition-colors ${active ? "bg-emerald-50/80 dark:bg-emerald-950/20" : "hover:bg-zinc-50 dark:hover:bg-zinc-900/70"}`}>
    <Link href={`/users/${encodeURIComponent(user.slug)}`} className="shrink-0 rounded-full focus:outline-none focus:ring-2 focus:ring-emerald-400" aria-label={`${user.name}-এর প্রোফাইল`}><Avatar user={user} /></Link>
    <Link href={`/users/${encodeURIComponent(user.slug)}`} className="min-w-0 flex-1 text-left focus:outline-none"><span className="flex items-center justify-between gap-2"><span className="truncate text-sm font-semibold text-zinc-800 dark:text-zinc-100">{user.name}</span><span className="shrink-0 text-[10px] text-zinc-400">{last ? time(last.created_at) : ""}</span></span><span className="mt-0.5 block truncate text-xs text-zinc-500 dark:text-zinc-400">{user.role_label || (last?.message || (last?.media?.length ? "সংযুক্তি" : "প্রোফাইল দেখুন"))}</span></Link>
    <button type="button" onClick={onMessage} className="shrink-0 rounded-lg bg-emerald-600 px-2.5 py-1.5 text-[11px] font-semibold text-white hover:bg-emerald-700" aria-label={`${user.name}-কে মেসেজ পাঠান`}>মেসেজ</button>
  </div>;
}
...
export default function ChatApp({ targetSlug }: { targetSlug?: string }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      if (!value) { setPeople([]); setSearching(false); return; }
      setSearching(true);
      try { const results = await searchUsers(value); if (requestId === searchRequestRef.current) setPeople(results); }
      catch (error) { if (requestId === searchRequestRef.current) { setPeople([]); toast.error(error instanceof Error ? error.message : "ব্যবহারকারী খোঁজা যায়নি"); } }
      finally { if (requestId === searchRequestRef.current) setSearching(false); }
    }, 300);
    return () => window.clearTimeout(timer);
  }, [query]);

  const loadMessages = async (slug: string, requestedPage = 1, append = false) => {
    if (requestedPage === 1) setLoadingMessages(true); else setMoreLoading(true);
    try { const data = await getMessages(slug, requestedPage, 30); setMessages(current => append ? [...data.data, ...current] : data.data.slice().reverse()); setPage(requestedPage); setHasMore(Boolean(data.next_page_url || requestedPage < (data.last_page || requestedPage))); }
    finally { setLoadingMessages(false); setMoreLoading(false); }
  };

  const selectUser = async (user: ChatUser) => {
    if (!user.slug) { toast.error("এই ব্যবহারকারীর বৈধ slug পাওয়া যায়নি"); return; }
    setSelected(user); setMobileChat(true); setPage(1); setMessages([]); setReplyTo(null); setEditing(null);
    try { const [blockState] = await Promise.all([getBlockStatus(user.slug), loadMessages(user.slug, 1)]); setBlocked(Boolean(blockState.blocked ?? blockState.is_blocked)); }
  ...
  useEffect(() => {
    ...
    const openTarget = async () => {
      try { const fromList = users.find(user => user.slug === targetSlug); const response = fromList ? null : await getUserProfileBySlug(targetSlug); const target = fromList ?? response?.data?.profile ?? response?.profile; if (!cancelled && target?.slug) { openedTargetRef.current = targetSlug; await selectUser(target); } }
  ...
  return <section className="w-full py-0 sm:py-5"><div className="mx-auto w-full max-w-7xl px-0 sm:px-4"><div className="grid h-[calc(100svh-4rem)] min-h-0 w-full grid-cols-1 overflow-hidden border-y border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-950 sm:h-[min(760px,calc(100svh-6rem))] sm:min-h-[620px] sm:rounded-2xl sm:border sm:shadow-sm md:grid-cols-[minmax(280px,340px)_minmax(0,1fr)]">
    <aside className={`${mobileChat ? "hidden md:flex" : "flex"} min-h-0 min-w-0 flex-col border-r border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-950`}>
      <div className="shrink-0 border-b border-zinc-200 px-4 py-4 dark:border-zinc-800"><div className="mb-3 flex items-center justify-between gap-3"><div><h1 className="text-lg font-bold tracking-tight text-zinc-900 dark:text-white">মেসেজ</h1><p className="mt-0.5 text-xs text-zinc-500">ব্যবহারকারী খুঁজে প্রোফাইল বা মেসেজ করুন</p></div><span className="rounded-full bg-emerald-50 px-2.5 py-1 text-[11px] font-medium text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300">{query ? people.length : users.length} জন</span></div><label className="flex h-10 items-center gap-2 rounded-xl border border-zinc-200 bg-zinc-50 px-3 transition focus-within:border-emerald-400 focus-within:bg-white dark:border-zinc-800 dark:bg-zinc-900 dark:focus-within:bg-zinc-950"><Search className="size-4 shrink-0 text-zinc-400" /><input value={query} onChange={e => setQuery(e.target.value)} placeholder="নাম, ইমেইল, ফোন বা slug খুঁজুন..." className="min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-zinc-400" /></label></div>
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">{searching ? <div className="flex justify-center py-10"><Loader2 className="size-5 animate-spin text-zinc-400" /></div> : query.trim() ? people.length ? <div className="py-1">{people.map(u => <SearchUserRow key={u.id} user={u} active={selected?.id === u.id} onMessage={() => void selectUser(u)} />)}</div> : <div className="px-6 py-16 text-center"><Search className="mx-auto mb-3 size-6 text-zinc-300" /><p className="text-sm font-medium text-zinc-600 dark:text-zinc-300">কোনো ব্যবহারকারী পাওয়া যায়নি</p><p className="mt-1 text-xs text-zinc-400">নাম, ইমেইল, ফোন বা slug দিয়ে আবার চেষ্টা করুন।</p></div> : users.length ? <div className="py-1">{users.map(u => <SearchUserRow key={u.id} user={u} active={selected?.id === u.id} onMessage={() => void selectUser(u)} />)}</div> : <div className="px-6 py-16 text-center"><Search className="mx-auto mb-3 size-6 text-zinc-300" /><p className="text-sm font-medium text-zinc-600 dark:text-zinc-300">কোনো কথোপকথন নেই</p></div>}</div>
    </aside>
    <main className={`${mobileChat ? "flex" : "hidden md:flex"} min-h-0 min-w-0 flex-col bg-zinc-50/70 dark:bg-zinc-950`}>
      {selected ? <><header className="flex h-[68px] shrink-0 items-center gap-3 border-b border-zinc-200 bg-white px-3 dark:border-zinc-800 dark:bg-zinc-950 sm:px-5"><button type="button" onClick={() => setMobileChat(false)} className="rounded-full p-2 text-zinc-500 hover:bg-zinc-100 md:hidden dark:hover:bg-zinc-900"><ArrowLeft className="size-5" /></button><Link href={`/users/${encodeURIComponent(selected.slug)}`} className="shrink-0 rounded-full focus:outline-none focus:ring-2 focus:ring-emerald-400"><Avatar user={selected} size="lg" /></Link><div className="min-w-0 flex-1"><Link href={`/users/${encodeURIComponent(selected.slug)}`} className="block truncate text-sm font-semibold text-zinc-900 hover:text-emerald-600 dark:text-white dark:hover:text-emerald-400">{selected.name}</Link><div className="mt-0.5 flex items-center gap-1.5 text-xs text-zinc-500"><span className={`size-1.5 rounded-full ${selected.is_online ? "bg-emerald-500" : "bg-zinc-300"}`} />{selected.is_online ? "অনলাইনে আছেন" : "অফলাইন"}</div></div><Link href={`/users/${encodeURIComponent(selected.slug)}`} className="rounded-full px-3 py-1.5 text-xs font-medium text-emerald-700 hover:bg-emerald-50 dark:text-emerald-300 dark:hover:bg-emerald-950/30">প্রোফাইল</Link><button type="button" disabled={blockLoading} onClick={() => void block()} className={`rounded-full p-2 transition ${blocked ? "bg-red-50 text-red-600 dark:bg-red-950/30" : "text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-900"}`}>{blockLoading ? <Loader2 className="size-5 animate-spin" /> : <ShieldBan className="size-5" />}</button></header>
