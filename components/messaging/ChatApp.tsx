...
function MessageBubble({ message, own, onReply, onEdit, onDelete }: { message: ChatMessage; own: boolean; onReply: () => void; onEdit: () => void; onDelete: () => void }) {
  ...
  return <div className={`group flex w-full ${own ? "justify-end" : "justify-start"}`}><div className={`max-w-[min(78%,620px)] ${own ? "items-end" : "items-start"} flex flex-col gap-1`}>
    {message.parent ? <button type="button" onClick={onReply} className="max-w-full rounded-lg border-l-2 border-zinc-400/50 bg-zinc-400/10 px-3 py-1.5 text-left text-xs opacity-70"><span className="font-semibold">{message.parent.sender?.name || "রিপ্লাই"}</span><span className="ml-1 line-clamp-2">{message.parent.message || "সংযুক্তি"}</span></button> : null}
    ...
    <div className="flex items-center gap-1 opacity-0 transition group-hover:opacity-100 focus-within:opacity-100"><button type="button" onClick={onReply} className="rounded-lg bg-zinc-400/10 px-2 py-1 text-[11px] opacity-70 hover:bg-zinc-400/25"><Reply className="mr-1 inline size-3" />রিপ্লাই</button>{own ? <><button type="button" onClick={onEdit} className="rounded-lg bg-zinc-400/10 px-2 py-1 text-[11px] opacity-70 hover:bg-zinc-400/25">এডিট</button><button type="button" onClick={onDelete} className="rounded-lg bg-zinc-400/10 px-2 py-1 text-[11px] opacity-70 hover:bg-zinc-400/25">মুছুন</button></> : null}</div>
  </div></div>;
}
...
function SearchUserRow({ user, active, onMessage }: { user: ChatUser; active: boolean; onMessage: () => void }) {
  ...
  return <div className={`group flex items-center gap-2 px-3 py-2.5 transition-colors ${active ? "bg-zinc-400/25" : "hover:bg-zinc-400/10"}`}>
    <Link href={`/users/${encodeURIComponent(user.slug)}`} className="shrink-0 rounded-full focus:outline-none focus:ring-2 focus:ring-zinc-400/50" aria-label={`${user.name}-এর প্রোফাইল`}><Avatar user={user} /></Link>
    ...
    <button type="button" onClick={onMessage} className="shrink-0 rounded-lg bg-zinc-400/25 px-2.5 py-1.5 text-[11px] font-semibold hover:bg-zinc-400/50" aria-label={`${user.name}-কে মেসেজ পাঠান`}>মেসেজ</button>
  </div>;
}
