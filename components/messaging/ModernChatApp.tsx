...
function Avatar({ user, large = false }: { user?: ChatUser | null; large?: boolean }) {
  ...
  return src ? <img src={src} alt={user?.name || ""} className={`${size} shrink-0 rounded-full object-cover ring-1 ring-black/10`} /> : <div className={`${size} shrink-0 rounded-full bg-zinc-200 dark:bg-zinc-800 flex items-center justify-center text-sm font-bold`}>{user?.name?.slice(0, 1)?.toUpperCase() || "?"}</div>;
}
...
function MessageBubble({ message, own, onReply, onEdit, onDelete }: { message: ChatMessage; own: boolean; onReply: () => void; onEdit: () => void; onDelete: () => void }) {
  ...
  return <div className={`group flex w-full ${own ? "justify-end" : "justify-start"}`}><div className={`flex max-w-[min(80%,620px)] flex-col gap-1 ${own ? "items-end" : "items-start"}`}>
    {message.parent ? <button type="button" onClick={onReply} className="max-w-full rounded-xl border-l-2 border-zinc-400/50 bg-zinc-100 px-3 py-1.5 text-left text-xs dark:bg-zinc-900"><span className="font-semibold">{message.parent.sender?.name || "রিপ্লাই"}</span><span className="ml-1 line-clamp-2 opacity-60">{message.parent.message || "সংযুক্তি"}</span></button> : null}
    <div className={`rounded-2xl px-3.5 py-2.5 shadow-sm ${own ? "rounded-br-md bg-zinc-900 text-white dark:bg-zinc-100 dark:text-zinc-900" : "rounded-bl-md border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900"}`}>
    ...
    <div className="flex items-center gap-1 opacity-0 transition group-hover:opacity-100 focus-within:opacity-100"><button type="button" onClick={onReply} className="rounded-lg bg-zinc-100 px-2 py-1 text-[11px] dark:bg-zinc-800"><Reply className="mr-1 inline size-3" />রিপ্লাই</button>{own ? <><button type="button" onClick={onEdit} className="rounded-lg bg-zinc-100 px-2 py-1 text-[11px] dark:bg-zinc-800">এডিট</button><button type="button" onClick={onDelete} className="rounded-lg bg-zinc-100 px-2 py-1 text-[11px] dark:bg-zinc-800">মুছুন</button></> : null}</div>
  </div></div>;
}
