...
export default function AdvancedBanglaCalendar() {
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    if (diff > 50)
      navigateMonth(1); // Swipe Left -> Next
  ...
  return (
    <div className="max-w-md mx-auto space-y-4 select-none relative">
      ...
      <header className="space-y-2">
        <h1 className="text-xl font-bold tracking-tight text-zinc-50 text-zinc-100">
          আজকের বাংলা তারিখ:{" "}
      ...
      <section
        ...
        className="rounded-2xl border border-zinc-400/25 p-4 cursor-pointer hover: transition-shadow group"
      >
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm text-zinc-400 mb-1">বাংলা তারিখ</p>
            <p className="text-2xl  text-zinc-300 group-hover:scale-105 transition-transform origin-left">
              {bnNum(selBn.day)} {selBn.month}
            </p>
          </div>
          <div className="text-right">
            <p className="text-sm text-zinc-400 mb-1">ইংরেজি তারিখ</p>
            <p className="text-xl font-bold text-zinc-50 text-zinc-200">
              {format(selectedDate, "dd MMMM yyyy")}
            </p>
          </div>
        </div>
        {selHoliday && (
          <div className="mt-4 pt-4 border-t border-zinc-100 border-zinc-400/25 flex justify-between items-center">
            <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-rose-100 text-rose-700 text-sm font-bold">
      ...
      <section className="flex items-center justify-between rounded-2xl bg-zinc-400/10/50 px-3 py-2 ">
        ...
        <div className="flex gap-2">
          ...
          <select
            ...
            className="bg-transparent font-bold text-zinc-50 text-zinc-200 focus:outline-none cursor-pointer appearance-none text-center"
          >
            {Array.from({ length: 12 }).map((_, i) => (
              <option key={i} value={i} className="text-zinc-50">
          ...
          <select
            ...
            className="bg-transparent font-bold text-zinc-50 text-zinc-200 focus:outline-none cursor-pointer appearance-none"
          >
            {Array.from({ length: 21 }).map((_, i) => {
              const y = new Date().getFullYear() - 10 + i;
              return (
                <option key={y} value={y} className="text-zinc-50">
                  {y}
                </option>
              );
            })}
      ...
      >
        <div className="grid grid-cols-7 mb-2">
          {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((wd, i) => (
            <div
              key={wd}
              className={`text-center py-1 text-xs font-bold ${i >= 5 ? "text-rose-500" : "text-zinc-400"}`}
            >
              {wd}
            </div>
          ))}
              ...
              return (
                ...
                >
                  <div className="flex flex-col items-center justify-center h-full">
                    <span
                      className={`text-sm font-black ${isSelected ? "text-white" : day.isWeekend ? "text-rose-500" : ""}`}
                    >
                      {day.engDay}
                    </span>
                    <span
                      className={`text-[11px] font-bold mt-0.5 ${isSelected ? "text-emerald-100" : "text-zinc-300"}`}
                    >
                      {bnNum(day.bnObj.day)}
                    </span>
                    {holiday && (
                      <span className="absolute bottom-1 w-1.5 h-1.5 rounded-full bg-rose-500 shadow-[0_0_4px_rgba(244,63,94,0.8)]" />
                    )}
                  </div>
          ...
          >
            ...
            >
              ...
              <div className="text-center space-y-4">
                <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-zinc-400/10 mb-2">
                  <Calendar className="w-8 h-8 text-zinc-300" />
                </div>

                <h3 className="text-2xl font-bold text-zinc-50 dark:text-white">
                  {bnNum(selBn.day)} {selBn.month}, {bnNum(selBn.year)}
                </h3>
      ...
      {/* --- Informative Content --- */}
      <section className="/50 p-4 rounded-2xl space-y-4">
        ...
        <p className=" text-sm leading-relaxed">
          ...
          বর্ষপঞ্জি। বাংলা সনের নতুন বছর শুরু হয় সাধারণত ১৪ এপ্রিল (পহেলা
          বৈশাখ) থেকে। বর্তমানে চলছে{" "}
          <strong>{bnNum(startMonthBn.year)} বঙ্গাব্দ</strong>।
        </p>
        <p className=" text-sm leading-relaxed">
      ...
      <section className="space-y-4 pt-4">
        ...
        <details className="group /50 rounded-xl [&_summary::-webkit-details-marker]:hidden">
          <summary className="flex cursor-pointer items-center justify-between gap-2 p-4 text-zinc-50 text-zinc-100 ">
        ...
        <details className="group /50 rounded-xl [&_summary::-webkit-details-marker]:hidden">
          ...
          <div className="px-4 pb-4 text-sm  leading-relaxed">
            বাংলা সন সাধারণত ১৪ এপ্রিল থেকে শুরু হয়। ইংরেজি বছর থেকে ৫৯৩ বা ৫৯৪
            বিয়োগ করে বাংলা সন পাওয়া যায়। এপ্রিলের ১৪ তারিখের আগে হলে ৫৯৪ এবং
            পরে হলে ৫৯৩ বিয়োগ করা হয়।
          </div>
        </details>
      </section>
    </div>
