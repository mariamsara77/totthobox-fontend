...
export default function SettingsPanel({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-md bg-zinc-200 dark:bg-zinc-900 h-full overflow-y-auto shadow-2xl p-5 space-y-7">
        {/* Header */}
        <div className="flex items-center justify-between sticky top-0 pb-3 z-10">
          <h2 className="text-xl font-semibold">সেটিংস</h2>
          <button
            type="button"
            onClick={onClose}
            className="size-9 flex items-center justify-center rounded-xl bg-zinc-400/10 hover:bg-zinc-400/20 transition"
          >
            ✕
          </button>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-3">
            <input
              ...
              className="size-11 rounded-xl cursor-pointer border border-zinc-600"
            />
        ...
        <div className="space-y-2">
          <h3 className="text-sm font-medium">
            ব্রাশের সাইজ: <span className="opacity-70">{currentSize}px</span>
          </h3>
          <input
            type="range"
            min={1}
        ...
        <div className="space-y-2">
          <h3 className="text-sm font-medium">
            স্বচ্ছতা:{" "}
            <span className="opacity-70">{Math.round(opacity * 100)}%</span>
          </h3>
          <input
            type="range"
            min={0.1}
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {CHARACTER_CATEGORIES.map((cat, index) => (
              <div
                key={cat.name}
                className="rounded-xl bg-zinc-400/10 overflow-hidden"
              >
                <button
                  type="button"
                  onClick={() => toggleCategory(index)}
                  className="w-full flex justify-between items-center p-3 hover:bg-zinc-400/15 text-left transition"
                >
                ...
                {openCategories[index] && (
                  <div className="p-3 grid grid-cols-6 gap-2 border-t border-zinc-400/20">
                    {cat.characters.map((char) => (
                      <button
                        ...
                        className="h-10 flex items-center justify-center rounded-lg bg-zinc-400/10 hover:bg-zinc-400/20 text-lg transition"
                      >
                        {char}
                      </button>
                    ))}
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2.5">
            {PAPER_STYLES.map((paper) => (
              ...
              >
                <div
                  className={`h-10 rounded-lg mb-1.5 border border-zinc-600 ${paper.preview}`}
                />
